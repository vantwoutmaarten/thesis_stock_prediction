...
for seed in range(seeds):
    # Create experiment
    neptune.create_experiment('7D_predictor_exp4_presence', params = PARAMS, upload_source_files=['./thesis_experiments/experiment4/LSTM_manager_7D.py', './thesis_experiments/experiment4/7D_predictor_presence.py'], tags=['single_run', '7D-prediction', '4-year', '20-step-ahead', '20-predictions', 'shifted30', 'presence', 'seed'+str(seed)])
    ...
    data = df.filter(items=[data_name, lagged_imputation_forwardfill, lagged_imputation_globalmean, lagged_imputation_meanlast30, lagged_imputation_linearfit30, lagged_imputation_cubicfit30, presence])
