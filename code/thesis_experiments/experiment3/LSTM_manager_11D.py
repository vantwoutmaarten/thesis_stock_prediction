...
class LSTM(nn.Module):	
    def __init__(self, input_size=11, hidden_layer_size=40, output_size=1, dropout = 0.0, num_layers=1):	
        super().__init__()
    ...
    def forward(self, input_seq):	
        ...
        input_seq = input_seq.view(len(input_seq), 1, 11)
...
class LSTMHandler():	
    ...
    def create_train_test_data(self, data = None,
     ...
     test_size=365
        ...
        self.train_data_normalized = torch.cuda.FloatTensor(train_data_normalized).view(-1,11)
    ...
    def make_predictions_from_model(self, modelpath=None, modelstate=None):	
        ...
        for i in range(fut_pred):	
            ...
            with torch.no_grad():	
                ...
                test_inputs.append([modeloutput, np.NAN, np.NAN,np.NAN,np.NAN,np.NAN, np.NAN, np.NAN, np.NAN, np.NAN, np.NAN])	
        actual_predictions = self.scaler.inverse_transform(np.array(test_inputs[-self.test_data_size:]).reshape(-1, 11))
